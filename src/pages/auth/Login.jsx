...
import { Mail, Lock, Eye, EyeOff, Sparkles, ArrowRight, ArrowLeft, ShieldCheck, AlertCircle, CheckCircle2 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-white dark:bg-[#0d071a] px-3 py-10 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="relative z-10 flex w-full max-w-5xl my-8 shadow-2xl shadow-indigo-500/10 dark:shadow-black/40 rounded-3xl overflow-hidden animate-on-load border-2 border-indigo-300/60 dark:border-purple-500/40 ring-4 ring-purple-200/30 dark:ring-purple-900/20">
        ...
        {/* Left Side (Desktop) - Ab hamesha Left hi rahega */}
        <div className="hidden md:flex md:w-1/2 bg-gradient-to-br from-purple-200 via-purple-100 to-indigo-100 dark:from-indigo-700 dark:via-purple-800 dark:to-violet-900 p-10 flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            {/* ✅ Arabic heading - bigger & bolder */}
            <div className="mt-6 text-left">
              <h1 className="text-5xl font-bold text-purple-900 dark:text-white leading-tight" style={arabicTextStyle}>
                <span className="block">{t("auth.login.title1")}</span>
                <span className="block text-indigo-700 dark:text-indigo-300" style={arabicTextStyle}>{t("auth.login.titleHighlight")}</span>
              </h1>
              {/* ✅ Arabic sub-text - bigger & bolder */}
              <p className="mt-4 text-lg text-indigo-700 dark:text-indigo-300" style={arabicSubTextStyle}>{t("auth.login.subtitle")}</p>
            </div>
          </div>
          
          {/* Security - Arabic mein bhi left hi rahega, bas text right-align */}
          <div className="relative z-10 mt-12">
            <div className="flex items-center gap-4 text-purple-900 dark:text-white/80">
              ...
              <div className={`${isRTL ? 'text-right' : 'text-left'}`}>
                {/* ✅ Arabic secure text - bigger & bolder */}
                <p className="text-lg font-semibold" style={arabicSubTextStyle}>{t("auth.login.secureReliable")}</p>
                <p className="text-sm text-indigo-700 dark:text-indigo-300" style={arabicSmallTextStyle}>{t("auth.login.dataProtected")}</p>
              </div>
            </div>
          </div>
        </div>

        {/* Right Side (Form) - ✅ RTL mein text right-aligned */}
        <div className={`w-full md:w-1/2 bg-white/90 dark:bg-[#18132a]/90 backdrop-blur-lg p-6 sm:p-10 flex flex-col justify-center ${isRTL ? 'text-right' : 'text-left'}`}>
          ...
          <div className="md:hidden mb-6 text-center">
            ...
            {/* ✅ Arabic mobile heading - bigger & bolder */}
            <h2 className={`mt-3 text-3xl font-bold text-slate-900 dark:text-white ${isRTL ? 'text-right' : 'text-left'}`} style={arabicTextStyle}>
              {t("auth.login.welcomeBack")} <span className="bg-gradient-to-r from-indigo-600 via-purple-600 to-violet-600 bg-clip-text text-transparent dark:from-indigo-300 dark:via-purple-300 dark:to-violet-300" style={arabicTextStyle}>{t("auth.login.back")}</span>
            </h2>
            {/* ✅ Arabic mobile sub-text - bigger */}
            <p className={`mt-2 text-sm text-slate-600 dark:text-slate-400 ${isRTL ? 'text-right' : 'text-left'}`} style={arabicSmallTextStyle}>{t("auth.login.signIn")}</p>
          </div>

          {/* Desktop Header - ✅ RTL right-aligned */}
          <div className={`hidden md:block ${isRTL ? 'text-right' : 'text-left'}`}>
            ...
            {/* ✅ Arabic desktop heading - bigger & bolder */}
            <h2 className="font-display text-3xl font-bold text-slate-900 dark:text-white" style={arabicTextStyle}>
              {t("auth.login.welcomeBack")} <span className="bg-gradient-to-r from-indigo-600 via-purple-600 to-violet-600 bg-clip-text text-transparent dark:from-indigo-300 dark:via-purple-300 dark:to-violet-300" style={arabicTextStyle}>{t("auth.login.back")}</span>
            </h2>
            {/* ✅ Arabic desktop sub-text - bigger */}
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-400" style={arabicSmallTextStyle}>{t("auth.login.signIn")}</p>
          </div>

          <form className="mt-6 md:mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
            <div className="space-y-5">
              <div>
                {/* ✅ RTL mein label right-aligned */}
                ...
                {emailError && <p className={`mt-1 text-xs text-red-500 flex items-center gap-1 ${isRTL ? 'justify-end' : ''}`} style={isRTL ? { fontSize: 'clamp(0.9rem, 1.2vw, 1rem)' } : {}}><AlertCircle className="h-3.5 w-3.5" /> {emailError}</p>}
              </div>
              ...
              <div>
                ...
                {passwordError && <p className={`mt-1 text-xs text-red-500 flex items-center gap-1 ${isRTL ? 'justify-end' : ''}`} style={isRTL ? { fontSize: 'clamp(0.9rem, 1.2vw, 1rem)' } : {}}><AlertCircle className="h-3.5 w-3.5" /> {passwordError}</p>}
              </div>

              {/* ✅ Remember me & Forgot password - RTL mein flex-row-reverse */}
              <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                <div className={`flex items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <label htmlFor="remember-me" className={`ml-2 block text-sm text-slate-600 dark:text-slate-400 ${isRTL ? 'mr-2 ml-0' : ''}`} style={isRTL ? { fontSize: 'clamp(0.95rem, 1.3vw, 1.1rem)' } : {}}>{t("auth.login.rememberMe")}</label>
                </div>
                <Link to="/forgot-password" onClick={handleForgotPassword} className="text-sm font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors duration-300" style={isRTL ? { fontSize: 'clamp(0.95rem, 1.3vw, 1.1rem)', fontWeight: '600' } : {}}>{t("auth.login.forgotPassword")}</Link>
              </div>
            </div>

            <div>
              {/* ✅ Button - bigger & bolder for Arabic */}
              <button type="submit" className="group relative w-full flex items-center justify-center gap-2 py-3 px-4 border border-transparent rounded-xl text-sm font-semibold text-white bg-brand-600 shadow-lg shadow-brand-500/25 transition-all duration-300 hover:bg-brand-700 hover:shadow-[0_0_15px_rgba(99,102,241,0.3)]" style={isRTL ? { fontSize: 'clamp(1rem, 1.3vw, 1.15rem)', fontWeight: '600' } : {}}>
                {t("auth.login.signInButton")} <ArrowIcon className="h-4 w-4" />
              </button>
            </div>

            {generalError && <div className={`flex items-center justify-center gap-2 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30 p-2 rounded-lg ${isRTL ? 'flex-row-reverse' : ''}`} style={isRTL ? { fontSize: 'clamp(0.95rem, 1.3vw, 1.1rem)' } : {}}><AlertCircle className="h-4 w-4" /> {generalError}</div>}

            {/* ✅ Sign up link - bigger for Arabic */}
            <div className={`text-center text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
              <span className="text-slate-600 dark:text-slate-400" style={isRTL ? { fontSize: 'clamp(0.95rem, 1.3vw, 1.1rem)' } : {}}>{t("auth.login.noAccount")} </span>
              <Link to="/signup" className="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors duration-300" style={isRTL ? { fontSize: 'clamp(0.95rem, 1.3vw, 1.1rem)', fontWeight: '600' } : {}}>{t("auth.login.signUpFree")}</Link>
            </div>
          </form>
        </div>
      </div>
