...
import { Sparkles, Zap, Infinity, ArrowRight, ArrowLeft, Clock, Smartphone } from "lucide-react";
  ...
  return (
    <section ref={ref} className="relative overflow-hidden px-4 py-24 sm:px-6 lg:px-8 lg:py-32 bg-gradient-to-br from-indigo-50 via-purple-50 to-white dark:bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] dark:from-[#2d1045] dark:via-[#0d071a] dark:to-[#0d071a]">
      ...
      <div className={`relative z-10 mx-auto max-w-7xl scroll-reveal ${isVisible ? 'visible' : ''}`}>
        ...
        <div className={`grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-16 ${isRTL ? 'lg:grid-flow-dense' : ''}`}>
          
          {/* LEFT SIDE: Text (AR mein Right) */}
          <div className={`flex flex-col justify-center gap-6 ${isRTL ? 'lg:order-2 text-right' : 'text-left'}`}>
            ...
            <h1 className={`font-display text-4xl font-bold leading-tight sm:text-5xl lg:text-6xl text-slate-900 dark:text-white ${isRTL ? 'text-right' : 'text-left'}`}>
            ...
            <p className={`max-w-lg text-lg leading-relaxed text-slate-800 dark:text-slate-100 ${isRTL ? 'text-right self-end' : 'text-left'}`}>
            ...
            <div className={`flex flex-wrap items-center gap-4 ${isRTL ? 'justify-end' : ''}`}>
              <div className={`inline-flex items-center gap-2 self-start rounded-full border border-indigo-200/30 bg-white/80 px-4 py-1.5 text-sm font-medium text-brand-600 backdrop-blur-sm dark:border-slate-700/50 dark:bg-slate-800/80 dark:text-slate-300 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                {t("pricingHero.badge1")}
              </div>
              <div className={`inline-flex items-center gap-2 self-start rounded-full border border-indigo-200/30 bg-white/80 px-4 py-1.5 text-sm font-medium text-brand-600 backdrop-blur-sm dark:border-slate-700/50 dark:bg-slate-800/80 dark:text-slate-300 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <Smartphone className="h-4 w-4" strokeWidth={2} />
                {t("pricingHero.badge2")}
              </div>
            </div>
            ...
            <div className={`mt-4 flex flex-wrap items-center gap-4 ${isRTL ? 'justify-end' : ''}`}>
              <Link
                to="/signup"
                className={`inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-indigo-600 to-violet-600 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/35 hover:brightness-110 dark:shadow-indigo-500/20 ${isRTL ? 'flex-row-reverse' : ''}`}
              >
          ...
          {/* RIGHT SIDE: Billing Box (AR mein Left) */}
          <div className={`relative flex items-center justify-center lg:justify-end animate-on-load ${isRTL ? 'lg:order-1' : ''}`}>
            <div 
              ...
              <div className={`bg-white/80 dark:bg-indigo-950/40 backdrop-blur-lg border border-indigo-200/50 dark:border-white/10 p-6 rounded-3xl shadow-xl shadow-indigo-500/5 dark:shadow-2xl shadow-black/30 ${isRTL ? 'w-[26rem]' : 'w-full max-w-md'}`}>
                ...
                <div className={`mb-6 ${isRTL ? 'text-right' : 'text-left'}`}>
                  <h3 className="text-xs font-semibold uppercase tracking-widest text-brand-600 dark:text-brand-400">
                    {t("pricingHero.chooseBilling")}
                  </h3>
                  <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                    {t("pricingHero.billingNote")}
                  </p>
                </div>
                ...
                <div className={`flex flex-col gap-4`}>
                  ...
                  >
                    <button
                      ...
                      className={`relative z-10 flex w-full items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}
                    >
                      <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        ...
                        <span className="font-medium text-slate-900 dark:text-white">
                          {t("pricingHero.monthly")}
                        </span>
                      </div>
                      <div className="text-lg font-bold text-slate-900 dark:text-white">
                        {t("pricingHero.monthlyPrice")}
                      </div>
                    </button>
                  </MouseFollower>
                  ...
                  >
                    ...
                    >
                      <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        ...
                        <span className="font-medium text-slate-900 dark:text-white">
                          {t("pricingHero.yearly")}
                        </span>
                      </div>
                      <div className="text-lg font-bold text-slate-900 dark:text-white">
                        {t("pricingHero.yearlyPrice")}
                      </div>
                    </button>
                  </MouseFollower>
                </div>
