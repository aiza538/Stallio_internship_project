  ...
  return (
    <section ref={ref} className={`relative overflow-hidden py-24 scroll-reveal ${isVisible ? 'visible' : ''}`}>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        {/* ✅ FIX: AR mein CTA Left, Accordion Right */}
        <div className={`grid grid-cols-1 lg:grid-cols-12 gap-10 ${isRTL ? 'lg:grid-flow-dense' : ''}`}>
          
          {/* LEFT SIDE (STICKY CTA) - AR mein Right */}
          <div className={`order-2 lg:order-none lg:col-span-4 ${isRTL ? 'lg:order-2' : ''}`}>
              ...
              <div className="relative rounded-3xl overflow-hidden p-8 shadow-2xl shadow-purple-500/20">
                ...
                <div className={`relative z-10 ${isRTL ? 'text-right' : 'text-left'}`}>
                  ...
                  <div className={`mt-6 ${isRTL ? 'text-right' : 'text-left'}`}>
                    <Link to="/contact" className={`inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-purple-600 transition-all duration-300 hover:shadow-xl hover:bg-purple-50 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      {t("pricingFaq.contactSupport")} <ArrowIcon className="h-4 w-4" />
                    </Link>
                  </div>
                </div>
              </div>
          ...
          {/* RIGHT SIDE (ACCORDION) - AR mein Left */}
          <div className={`order-1 lg:order-none lg:col-span-8 ${isRTL ? 'lg:order-1' : ''}`}>
                  ...
                  >
                    ...
                    >
                      <button
                        ...
                        className={`flex w-full items-center justify-between p-5 ${isRTL ? 'flex-row-reverse text-right' : 'text-left'}`}
                      >
                      ...
                      >
                        <p className={`px-5 pb-5 pl-19 text-base leading-relaxed text-slate-600 dark:text-slate-300 ${isRTL ? 'text-right' : 'text-left'}`}>
                          {t(faq.answerKey)}
                        </p>
                      </div>
                    </div>
