...
import { Store, Package, Link, ArrowRight, ArrowLeft, Zap, Clock, Sparkles } from "lucide-react";
...
const STEPS_COLORS = [
  {
    ...
    lineColor: "bg-amber-500 dark:bg-amber-400", // ✅ Line ka color
  },
  {
    ...
    lineColor: "bg-blue-500 dark:bg-blue-400", // ✅ Line ka color
  },
  {
    ...
    lineColor: "bg-purple-500 dark:bg-purple-400", // ✅ Line ka color
  },
];
    ...
    <section ref={ref} className="relative overflow-hidden px-4 py-16 sm:px-6 lg:px-8 lg:py-24 bg-[#faf7ff] dark:bg-transparent">
      ...
      <div className={`relative mx-auto max-w-content scroll-reveal ${isVisible ? 'visible' : ''}`}>
        ...
        <div className="relative grid grid-cols-1 gap-12 md:grid-cols-3">
          ...
          {/* Connecting Line (Desktop Only) - RTL mein colors flip honge */}
          <div className={`absolute top-12 hidden h-0.5 z-10 md:block ${isRTL ? 'left-[15%] right-[15%]' : 'left-[15%] right-[15%]'}`}>
            {/* Segment 1 (Yellow) - EN/ES: Left | AR: Right */}
            <div className={`absolute top-0 h-full w-1/3 bg-amber-500 dark:bg-amber-400 ${isRTL ? 'right-0' : 'left-0'}`} />
            {/* Segment 2 (Blue) - Middle */}
            <div className="absolute left-1/3 top-0 h-full w-1/3 bg-blue-500 dark:bg-blue-400" />
            {/* Segment 3 (Purple) - EN/ES: Right | AR: Left */}
            <div className={`absolute top-0 h-full w-1/3 bg-purple-500 dark:bg-purple-400 ${isRTL ? 'left-0' : 'right-0'}`} />
          </div>
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={index} className={`relative flex flex-col items-center ${isRTL ? (index === 0 ? 'md:order-3' : index === 1 ? 'md:order-2' : 'md:order-1') : ''}`}>
                ...
                >
                  <div className="relative flex flex-col items-center justify-center text-center">
                    ...
                    <div className={`mt-4 inline-flex items-center gap-1.5 rounded-full border-2 border-indigo-200/50 bg-white/50 px-3 py-1 text-xs font-semibold text-indigo-700 dark:border-indigo-800/30 dark:bg-indigo-950/20 dark:text-indigo-300 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                {index < steps.length - 1 && (
                  <div className={`absolute top-1/2 hidden -translate-y-1/2 md:block ${isRTL ? '-left-4' : '-right-4'}`}>
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-lg dark:bg-slate-800">
                      <ArrowIcon className="h-4 w-4 text-indigo-500 dark:text-indigo-400" strokeWidth={2.5} />
                    </div>
                  </div>
                )}
              </div>
